...
一个全 Serverless 系统，持续监控 **34 个 AWS 商业 Region**，覆盖 7 个风险维度，计算地缘政治风险指数（GPRI, 0–100），在风险等级变化时触发告警 —— 帮助运维团队**提前做好容灾准备**。

## 为什么需要？

传统 DR 方案是被动的 —— 事故发生后才启动应急响应。本系统并不预测 AWS 故障，而是持续监控**影响 Region 稳定性的地缘政治、环境和网络状况**。通过将这些信号量化为统一的风险指数（GPRI），帮助运维团队**提前感知风险、加固容灾准备**：何时预热备用 Region、降低 DNS TTL、演练故障切换流程 —— **在危机迫使你行动之前主动出击**。

- 🌊 巴林附近海缆被切断 → GPRI 上升 → **团队预热备用 Region，复查切换手册**
- 🌪️ 台风逼近东京 → GPRI 上升 → **团队评估是否需要将流量切换到备用 Region**
- 🔒 对某国实施新制裁 → GPRI 标记合规风险 → **团队提前审视合规暴露，而非事后补救**
...
A 类信号拥有**最高权重（20 分）**，因为武装冲突是 AWS Region 中断最强的预测指标。数据源优先级链反映了**覆盖度、时效性和可用性**之间的权衡：

| 优先级 | 数据源 | 优势 | 局限 |
|--------|--------|------|------|
| 第 1 | **ACLED** | 冲突数据的金标准。事件级粒度（战斗、爆炸、针对平民的暴力）。每日更新，支持按国家查询。联合国、世界银行和主流地缘政治风险平台均在使用。 | 需要 **Research+ 套餐**（机构邮箱注册）。免费 Gmail 账号只能获得 Open 套餐 = 无 API 权限。 |
| 第 2 | **UCDP + GDELT**（合并） | UCDP：学术严谨，精确的伤亡数据。GDELT：实时（15 分钟更新），免费无认证。两者合并覆盖面更广。 | UCDP 数据延迟约 1 年（学术出版周期）。GDELT 基于媒体报道 — 偏向英语报道，中东/非洲覆盖不足。 |
| 第 3 | **UCDP 单独** | 仍可用于历史基线和趋势分析。 | 无实时数据。 |
| 第 4 | **GDELT 单独** | 实时兜底方案。 | 国家覆盖有限（全球查询返回 250 篇文章，小国被美国/中国/印度淹没）。 |

> **⚠️ 没有 ACLED，中东地区（UAE、巴林）的 A 类分数将接近零** — UCDP 数据延迟约 1 年，GDELT 的全局查询无法捕获足够的区域冲突事件。**强烈建议在生产环境中配置 ACLED 凭证（Research+ 套餐）。**

系统还应用了**邻国溢出效应**：叙利亚的冲突会通过距离加权传播提升以色列的 A 类分数（NEIGHBOR_MAP 覆盖 12 个国家）。

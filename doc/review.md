...
TDD 第 1 节："目标账号 YOUR_ACCOUNT_ID，主 Region ap-northeast-1"。但第 2 节和 CDK `app.py` 都明确是 `us-west-2`。这个矛盾需要澄清——ap-northeast-1 应该是指被监控的"关注 Region"，不是部署 Region，但文档容易误导。
